...
const W={
 powerball:{m2_06_sum_band:.1587839135,m2_08_spacing_pattern:.1587839135,m2_05_parity_structure:.1270271308,m1_14_sum_conditional:.1239009799,m1_06_recency:.1164415366,m1_13_parity_conditional:.0996939665,m2_14_regime_structure:.0912290958},
...
function featureSet(rows,n){const short=countNums(rows.slice(-40),n),medium=countNums(rows.slice(-120),n),long=countNums(rows.slice(-300),n),global=countNums(rows,n),expo=countNums(rows,n,80),g=gaps(rows,n),pair=pairScores(rows,n),par=parityScore(rows,n),range=rangeScore(rows,n),pos=positionScore(rows,n),adj=adjacencyScore(rows,n),sum=sumBand(rows,n),rec=recurrence(rows,n),sig=signature(rows,n),recent=countNums(rows.slice(-40),n),older=countNums(rows.slice(-200,-40),n),reg={};for(let x=1;x<=n;x++)reg[x]=clamp(.65*recent[x]+.35*older[x]-.25*Math.abs(recent[x]-older[x]));return {short,medium,long,global,expo,g,pair,par,range,pos,adj,sum,rec,sig,reg:normObj(reg,n)}}
function methodScore(id,x,f,rows){const s=f.structural;switch(id){case'm1_01_global_frequency':return f.global[x];case'm1_02_short_frequency':return f.short[x];case'm1_03_medium_frequency':return f.medium[x];case'm1_04_long_frequency':return f.long[x];case'm1_05_exponential_frequency':return f.expo[x];case'm1_06_recency':{let since=rows.length;for(let i=rows.length-1;i>=0;i--)if(rows[i][2].includes(x)){since=rows.length-1-i;break}return Math.exp(-since/12)}case'm1_07_gap_pressure':return f.g.pressure[x];case'm1_08_gap_zscore':return f.g.z[x];case'm1_09_pair_association':return f.pair.assoc[x];case'm1_10_pair_transition':return f.pair.trans[x];case'm1_11_position_density':return f.pos[x];case'm1_12_range_density':return f.range[x];case'm1_13_parity_conditional':return f.par[x];case'm1_14_sum_conditional':return f.sum[x];case'm1_17_transition':return clamp(.55*f.pair.trans[x]+.45*f.rec[x]);case'm2_01_structural_draw_templates':return clamp(.45*s.sig[x]+.3*s.sum[x]+.25*s.par[x]);case'm2_02_pair_graph':return f.pair.graph[x];case'm2_04_range_structure':return s.range[x];case'm2_05_parity_structure':return s.par[x];case'm2_06_sum_band':return s.sum[x];case'm2_07_adjacency_structure':return s.adj[x];case'm2_08_spacing_pattern':return clamp(.6*s.adj[x]+.4*s.pos[x]);case'm2_09_position_transition':return clamp(.55*s.pos[x]+.45*f.pair.trans[x]);case'm2_10_recurrence_structure':return f.rec[x];case'm2_11_signature_similarity':return s.sig[x];case'm2_12_normalized_number_density':return clamp(.5*s.density[x]+.5*s.range[x]);case'm2_13_feature_interactions':return Math.sqrt(Math.max(0,f.short[x]*f.g.pressure[x]*f.pair.trans[x]));case'm2_14_regime_structure':return f.reg[x];default:return 0}}
// Numeric weights are explicit heuristics, not calibrated winning probabilities.
// Dates validate history in era_history.js; the scoring functions read numbers only.
function rootScores(rows,n){
 const counts=Array(10).fill(0),members=Array(10).fill(0),out={};
 for(let x=1;x<=n;x++)members[digitalRoot(x)]++;
 rows.forEach(row=>row[2].forEach(x=>counts[digitalRoot(x)]++));
 const rates=counts.map((count,root)=>members[root]?count/members[root]:0),maximum=Math.max(...rates,1);
 for(let x=1;x<=n;x++)out[x]=rates[digitalRoot(x)]/maximum;
 return out;
}
function bonusRanking(rows,n){
 const bonusRows=rows.filter(row=>Number.isInteger(row[3])&&row[3]>=1&&row[3]<=n).map(row=>[row[0],row[1],[row[3]]]);
 if(!bonusRows.length)return [];
 const frequency=countNums(bonusRows,n),recent=countNums(bonusRows.slice(-40),n),roots=rootScores(bonusRows,n);
 return Array.from({length:n},(_,i)=>{
  const number=i+1,contributions=[
   {method:'bonus_frequency',value:frequency[number],weight:.50},
   {method:'bonus_recent_frequency',value:recent[number],weight:.25},
   {method:'individual_droot_frequency',value:roots[number],weight:.25}
  ].map(term=>({...term,contribution:term.value*term.weight}));
  return {number,droot:digitalRoot(number),score:contributions.reduce((sum,term)=>sum+term.contribution,0),contributions,draws:bonusRows.length};
 }).filter(item=>item.score>0).sort((a,b)=>b.score-a.score||a.number-b.number);
}
function numericProfile(rows){
 const counts={tsum:{},setDroot:{},q:{},rootTransition:{}};
 const add=(group,value)=>counts[group][value]=(counts[group][value]||0)+1;
 let previous=null;
 for(const row of rows){
  const tsum=row[2].reduce((a,b)=>a+b,0),root=digitalRoot(tsum);
  add('tsum',tsum);add('setDroot',root);
  if(previous){add('q',Math.abs(tsum-previous.tsum));add('rootTransition',previous.root+'>'+root);}
  previous={draw:row[0],tsum,root};
 }
 return {counts,maxima:Object.fromEntries(Object.entries(counts).map(([key,values])=>[key,Math.max(0,...Object.values(values))])),previous};
}
function numericSetEvidence(numbers,profile){
 const tsum=numbers.reduce((a,b)=>a+b,0),setDroot=digitalRoot(tsum),previous=profile.previous;
 const values={tsum,setDroot,...(previous?{q:Math.abs(tsum-previous.tsum),rootTransition:previous.root+'>'+setDroot}:{})};
 const terms=Object.entries(values).map(([pattern,value])=>{
  const count=profile.counts[pattern][value]||0,maximum=profile.maxima[pattern];
  return {pattern,value,count,maximum,support:maximum?count/maximum:0};
 });
...
function rank(game,options={}){
 ...
 const total=Object.values(checkpoint).reduce((sum,w)=>sum+w,0);
 const weights=Object.fromEntries(Object.entries(checkpoint).map(([method,w])=>[method,.90*w/total]));
 weights.individual_droot_frequency=.10;
 const f=featureSet(rows,cfg.n);f.structural=PLATO_HISTORY.structuralFeatures(history.annotated,cfg.n,cfg.r);
 const roots=rootScores(rows,cfg.n),out=[];
 for(let x=1;x<=cfg.n;x++){
  const contributions=Object.entries(weights).map(([method,weight])=>{
   const value=method==='individual_droot_frequency'?roots[x]:methodScore(method,x,f,rows);
   if(!Number.isFinite(value))throw new Error('Invalid numeric feature: '+method);
   return {method,value,weight,contribution:weight*value};
  });
...
function setPatterns(metrics,previous){
 ...
 if(previous){keys.root_transition=previous.setDigitalRoot+'>'+metrics.setDigitalRoot;keys.transition=String(metrics.numbers.filter(x=>previous.numbers.includes(x)).length);keys.regimes=metrics.tsum>previous.tsum?'rising':metrics.tsum<previous.tsum?'falling':'unchanged';keys.q_t=String(Math.abs(metrics.tsum-previous.tsum));}
 return keys;
}
...
function analysePatterns(game,options={}){
 ...
 for(const [start,{era,rows}] of buckets){
  ...
  rows.forEach((row,index)=>{
   const metrics=describeSet(row[2],era.n),ordered=[...row[2]].sort((a,b)=>a-b),keys=setPatterns(metrics,sets.at(-1));
...
function makePortfolio(game,count){
 if(!window.PLATO_V35_COVERAGE)throw new Error('The number-only selector is not loaded.');
