...
test('Prize Coverage generates requested valid unique tickets for every game',()=>{
 for(const game of Object.keys(D.GAME_CFG))for(const count of [1,20,50]){
  const result=C.portfolio(game,count),cfg=D.GAME_CFG[game];assert.equal(result.tickets.length,count);
...
test('root has exactly three controls; no iframe or legacy UI boot path',()=>{
 const html=fs.readFileSync(path.join(root,'index.html'),'utf8');
 assert.equal((html.match(/<(?:select|input|button)\b/g)||[]).length,3);
 ...
 assert.match(html,/Game<\/label>/);assert.match(html,/Ticket count<\/label>/);assert.match(html,/Generate v3\.5/);
