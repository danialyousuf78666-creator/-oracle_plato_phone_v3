...
if pgrep -f "streamlit run app.py" >/dev/null 2>&1; then
  echo "Hunter Workstation already running on port 8501"
  exit 0
fi
...
PY
  then
    echo "Hunter Workstation ready on port 8501"
    exit 0
  fi
  sleep 1
done
echo "Hunter failed to become ready; inspect hunter_workstation/runs/streamlit.log" >&2
exit 1
